...
class AI_Mlp3(PLAYER.Player):
    ...
    def get_best_move(self, board):
            ...
            for c in range(board.width):
                ...
                if self.color == piece.color:
                    ...
                    for end_coordinate in valid_available_coordinates:
                        ...
                        prediction = self.model.predict(np.array(copy_board_data))
                        prediction = np.round(prediction, 3)
                        move_and_evaluations.append([[r, c], end_coordinate, prediction])
        ...
        for me in move_and_evaluations:
            ...
            if self.color == 'white':
                value = 1*evaluation[0] + 1.2*evaluation[1] + 1.4*evaluation[2] + 1.6*evaluation[3] + 1.8*evaluation[4]
            else:
                value = 1.8*evaluation[0] + 1.6*evaluation[1] + 1.4*evaluation[2] + 1.2*evaluation[3] + 1*evaluation[4]
