...
class Board:
    ...
    def __init__(self):
        ...
        self.turn_count = 1
        self.turn_since_capture_or_pawn_move = 0
    ...
    def copy_board_object(self):
        ...
        board.turn_since_capture_or_pawn_move = self.turn_since_capture_or_pawn_move
    ...
    def set_piece(self, piece, chess_file, chess_rank, debug=False):
        ...
        # If new piece is not null, update game data
        if piece.name != 'null':
            if piece.name == "King":
                self._has_king = True
                try:
                ...
                self.turn_since_capture_or_pawn_move = -1
            if piece.color == 'white':
        ...
        # If capturing piece is not null, update game data
        square = self.grid[row][col]
        if square.has_piece():
            self.turn_since_capture_or_pawn_move = -1
            if square.piece.color == 'white':
                index = self.white_pieces.index(square.piece)
                self.white_pieces.pop(index)
            else:
                index = self.black_pieces.index(square.piece)
    ...
    def get_valid_available_coordinates(self, piece):
        ...
        for coordinate in available_coordinates:
            ...
            copied_grid = self._copy_grid()
            saved_grid = self.grid
            self.grid = copied_grid
